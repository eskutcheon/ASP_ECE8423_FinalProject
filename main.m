...
function filter_state = initialize_filter_state(filter_func, filter_order, ref_noise, noisy_signal)
    switch func2str(filter_func)
        case 'rls_filter'
            % RLS Filter state
            filter_state = struct(
                'w', zeros(filter_order, 1), ...     % Filter weights
                'P', 10000 * eye(filter_order), ...  % Inverse covariance matrix
                'lambda', 1                          % Forgetting factor
            );

        case 'kalman_rls_filter'
            % Compute variances for Q1 and Q2
            Q1 = var(ref_noise) * eye(filter_order);   % Process noise covariance
            Q2 = var(noisy_signal) * eye(filter_order); % Measurement noise covariance

            % Kalman Filter state
            filter_state = struct(
                'x', zeros(filter_order, 1), ...      % Initial state
                'K', 10000 * eye(filter_order), ...  % Initial covariance matrix
                'Q1', Q1, ...                        % Process noise covariance
                'Q2', Q2, ...                        % Measurement noise covariance
                'F', eye(filter_order), ...          % State transition matrix
                'C', eye(filter_order)               % Measurement matrix
            );
...
function [output_signal, error_signal] = run_noise_canceller(filter_func, M, lambda, delta, ref_noise, noisy_signal)
    ...
    for iter = 1:10
        for n = M:N
            ...
            [filter_state, output_signal(n), error_signal(n)] = filter_func('adapt', M, N, lambda, delta, filter_state, input_signal, desired);
...
for i = 1:length(filters)
    ...
    output_signal = noisy_signal - output;
